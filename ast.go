...
func astMapType(e *ast.MapType, idents Identifiers) (r Value, err *posError) {
	k, err := astExprAsType(e.Key, idents)
	if err != nil {
		return
	}
	v, err := astExprAsType(e.Value, idents)
	if err != nil {
		return
	}
...
func astStructType(e *ast.StructType, idents Identifiers) (r Value, err *posError) {
	...
	for _, fs := range e.Fields.List {
		...
		fsT, err = astExprAsType(fs.Type, idents)
		if err != nil {
			return
		}
...
func astInterfaceType(e *ast.InterfaceType, idents Identifiers) (r Value, err *posError) {
	if e.Methods == nil {
...
func astExpr(e ast.Expr, idents Identifiers) (r Value, err *posError) {
	...
	case *ast.MapType:
		return astMapType(v, idents)
	case *ast.StructType:
		return astStructType(v, idents)
	case *ast.InterfaceType:
		return astInterfaceType(v, idents)
	default:
