...
func TestAstSelectorExpr(t *testing.T) {
	...
	if r, err := astSelectorExpr(&e, nil); r != nil || err == nil {
...
func TestAstBasicLit(t *testing.T) {
	...
	if r, err := astBasicLit(&e, nil); r != nil || err == nil {
		t.Errorf("expect %v %v, got %v %v", nil, true, r, err)
	}
}
...
func TestAstExpr(t *testing.T) {
	if r, err := astExpr(nil, nil); r != nil || err == nil {
		t.Errorf("expect %v %v, got %v %v", nil, true, r, err)
	}
	if r, err := astExpr(&ast.BadExpr{}, nil); r != nil || err == nil {
		t.Errorf("expect %v %v, got %v %v", nil, true, r, err)
	}
}
...
func TestAstStructType(t *testing.T) {
	...
	//
	e := &ast.StructType{}
	if r, err := astStructType(e, nil); r != nil || err == nil {
		t.Errorf("expect %v %v, got %v %v", nil, true, r, err)
	}
	...
	//	2
	//
	expr, err := parser.ParseExpr("struct{X int `potobuf:1`}")
	if err != nil {
		t.Fatal("expect no error")
	}
	e = expr.(*ast.StructType)
	...
	//	3
	//
	expr, err = parser.ParseExpr("struct{int `potobuf:1`}")
	if err != nil {
		t.Fatal("expect no error")
	}
	e = expr.(*ast.StructType)
...
func TestAstInterfaceType(t *testing.T) {
	e := &ast.InterfaceType{}
	if r, err := astInterfaceType(e, nil); r != nil || err == nil {
		t.Errorf("expect %v %v, got %v %v", nil, true, r, err)
	}
}
